...
## Browser preview / Telegram mode

The app works in a normal browser without Telegram. In browser preview it does not load `telegram-web-app.js`, so the console stays cleaner and the user is created as a local demo buyer.

Use a local server, not a direct double-click on `index.html`:
...
For GitHub Pages, deploy the built `dist` folder through GitHub Actions. Do not set Pages to serve the repository root.
